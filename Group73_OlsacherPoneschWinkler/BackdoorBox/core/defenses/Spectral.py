...
class Spectral(Base):
    ...
    def filter(self, schedule):
        ...
        # Optimized with DataLoader for batch processing
        subset = Subset(poisoned_trainset, cur_indices)
        
        # Determine batch size from schedule or default
        batch_size = 128
        num_workers = self.current_schedule.get('num_workers', 4)
        
        loader = DataLoader(subset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
        
        acts = []
        def layer_hook(module, inp, out):
            acts.append(out.data.view(out.data.size(0), -1).cpu())
        
        # Assuming layer4 exists and is the feature layer (ResNet18 standard)
        ...
        if len(acts) > 0:
            full_cov = torch.cat(acts, dim=0).numpy()
        else:
            full_cov = np.zeros((0,0)) # handle edge case
        ...
        # --- remove: map subset indices -> global dataset indices ---
        removed_global = np.array([cur_indices[j] for j in top_scores], dtype=np.int64)

        print('removed_inds_length:' + str(len(removed_global)))
        # print(removed_global)

        # --- left/keep: remove those global indices from the full training set ---
        filtered_benign_id = np.delete(np.arange(len(poisoned_trainset)), removed_global)

        print('left_inds_length:' + str(len(filtered_benign_id)))
        # print(filtered_benign_id)

        # Return GLOBAL removed indices + GLOBAL kept indices
        return removed_global, filtered_benign_id
